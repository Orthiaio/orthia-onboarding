    ...
    <TeamShell>
      ...
      {showLogTime && (
        <LogTimeModal
          ...
          taskLabel={`${project.key}-${task.number}: ${task.title}`}
      ...
      {task.blocked && (
        <div className="mb-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          ...
          {task.blocked_reason && <span>— {task.blocked_reason}</span>}
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
        ...
        <aside className="space-y-4 rounded-xl border border-slate-200 bg-white p-5">
          ...
          <SidebarField label="Blocked">
            {canEdit ? (
              <BlockedEditor
                blocked={task.blocked}
                reason={task.blocked_reason}
                onChange={(blocked, reason) =>
                  patch({ blocked, blocked_reason: reason })
                }
              />
            ) : task.blocked ? (
    ...
    >
      ...
      >
        <div>
          ...
          <p className="mt-1 truncate text-xs text-slate-500">{taskLabel}</p>
        </div>
        <div className="grid grid-cols-[1fr_6rem] gap-3">
          ...
          <label className="block">
            <span className="block text-xs font-medium text-slate-600">Hours</span>
            <input
              type="number"
              step="0.25"
              min="0.25"
...
function BlockedEditor({
  ...
  blocked: boolean;
  reason: string | null;
  onChange: (blocked: boolean, reason: string | null) => void;
}) {
      ...
      {editing ? (
        ...
        <>
          <div className="flex items-start gap-1.5">
            ...
            {reason && <span className="text-xs text-slate-700">{reason}</span>}
          </div>
  ...
  switch (a.action) {
    ...
    case "blocker_added":
      return typeof meta.reason === "string" && meta.reason
        ? `marked this blocked — ${meta.reason}`
        : "marked this blocked";
